  ...
  stages {
    ...
    stage('Master-Deploy') {
      ...
      steps {
        ...
        sh 'sshpass -p $TOURNEYGENPASSWORD scp -r -oStrictHostKeyChecking=no $WORKSPACE/DockerfileApp tourneygen@$SERVER:$TOURNEYGENBACKLOCATION/'
        sh 'sshpass -p $TOURNEYGENPASSWORD scp -r -oStrictHostKeyChecking=no $WORKSPACE/DockerfileMongo tourneygen@$SERVER:$TOURNEYGENBACKLOCATION/'
